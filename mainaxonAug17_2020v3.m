...
timesteps = 400; % number of time steps. duration = timesteps*delT
endlength = 200;%840; % total number of positions
...
h = 1e-1; % eventually time-varying
...
ratio = .3;%0;%.5e1 % weakening the coupling to see the effect. %for very small ratios (1e-3) there is no interaction depolarization. for 1e-1 we begin to see interaction depolarization and for 1e0 there is full interaction depolarization
...
r0 = r0*gcostheta; % To account for the geometry.
...
axon = packagedCNNDec24_2016v4(N, d, ell, L, delT, timesteps, endlength, temperature, Ranvier_array, r0, cm, gm, cnd, gnd, rf, InjectionDuration, injectnumber1, stimpos1, delaydelta, injectnumber2, stimpos2, delaydelta2, injectnumber3, stimpos3, delaydelta3, injectnumber4, stimpos4, Eonoff, Estrength, K3_1);
Timespent = toc
LTminus1 = timesteps;
% 
% % for count = 1:N
