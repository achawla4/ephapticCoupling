...
function returncellarray = poremovieApril23methodmod2(start, width, trialnum,b, loops,fieldzero, conductivity)
...
sourcelowv = [0:numchan-1]*pi/numchan; % single ion channel position in ring
sourcehighv = sourcelowv+(pi/numchan);% phi-width of the channel is added to get the upper limit.
Inot = 2e-12;
%2e-12/((pi/numchan)*rho1prime*1e-5); % peak current strength at surface of node is 2 pico Amperes. Thickness of 
% nodal cylindrical patch that acts as the source is taken to be 10 micron
%  roughly. radius is rho1prime. Thus Inot is the peak current per
...
for channelnum = 0:numchan-1

for i = 1:mylength
    for j = 1:mylength
        [Q, Q2] = zandrhointegralsmod2(sourcelowv(channelnum+1), sourcehighv(channelnum+1),pxnum(i,j),pi/2,pynum(i,j));
