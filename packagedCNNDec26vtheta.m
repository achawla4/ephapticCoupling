...
%%% Rev: Dec 27, 2015 %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function axon = packagedCNNDec26vtheta(N, d, ell, L, delT, timesteps, endlength, temperature, Ranvier_array, r0, cm, gm, cnd, gnd, rf, InjectionDuration, injectnumber1, stimpos1, delaydelta, injectnumber2, stimpos2, delaydelta2, injectnumber3, stimpos3, delaydelta3, injectnumber4, stimpos4, Eonoff, Estrength, K, theta)
...
for i = 1:N
    for j = 1:N
        ...
        if i==j
         ...
         bundle1.rf(1,j) = bundle1.rf/theta(1,j);
            alpha_0(1,j) = bundle1.r0/bundle1.rf(1,j) ; 
alpha_N(1,j) = alpha_0(1,j)/(1 + bundle1.N*alpha_0(1,j));
            
            
         b(i,j) = bundle1.cm/bundle1.delT + (1-alpha_N(1,j))/(bundle1.rf(1,j)*(bundle1.delX)^2) + bundle1.gm/2;
         
         
              B(i,j) = b(i,j);
            B(i,j) = (B(i,j)- (bundle1.cm/bundle1.delT) - (1/(bundle1.rf(1,j)*(bundle1.delX)^2)) - (bundle1.gm/2))*K(i,j) + (bundle1.cm/bundle1.delT + (1/(bundle1.rf(1,j)*(bundle1.delX)^2)) + bundle1.gm/2);
        else
             bundle1.rf(1,j) = bundle1.rf/theta(1,j);
            alpha_0(1,j) = bundle1.r0/bundle1.rf(1,j) ; 
alpha_N(1,j) = alpha_0(1,j)/(1 + bundle1.N*alpha_0(1,j));
             ee(i,j) = alpha_N(1,j)/(2*bundle1.rf(1,j)*(bundle1.delX)^2);
             B(i,j) = -2*ee(i,j);
            B(i,j) = B(i,j)*K(i,j);
        end
    end
         ...
         % create the Asubd subblock
            ...
            alpha_0(1,j) = bundle1.r0/bundle1.rf(1,j) ; 
alpha_N(1,j) = alpha_0(1,j)/(1 + bundle1.N*alpha_0(1,j));
a(i,j) = (alpha_N(1,j)-1)/(2*bundle1.rf(1,j)*(bundle1.delX)^2);
A(i,j) = a(i,j);
                        A(i,j) = (A(i,j) + (1/(2*bundle1.rf(1,j)*(bundle1.delX)^2)))*K(i,j) - (1/(2*bundle1.rf(1,j)*(bundle1.delX)^2));
                    else
                        bundle1.rf(1,j) = bundle1.rf/theta(1,j);
            alpha_0(1,j) = bundle1.r0/bundle1.rf(1,j) ; 
alpha_N(1,j) = alpha_0(1,j)/(1 + bundle1.N*alpha_0(1,j));
...
%               end
        ...
        a_nd = a;
         ...
         for i = 1:N
            ...
            alpha_0(1,j) = bundle1.r0/bundle1.rf(1,j) ; 
alpha_N(1,j) = alpha_0(1,j)/(1 + bundle1.N*alpha_0(1,j));
 b_nd(i,j) = bundle1.cnd/bundle1.delT + (1-alpha_N(1,j))/(bundle1.rf(1,j)*(bundle1.delX)^2);
 B_nd(i,j) = b_nd(i,j);
                     B_nd(i,j) = (B_nd(i,j) - (bundle1.cnd/bundle1.delT) -(1/(bundle1.rf(1,j)*(bundle1.delX)^2)))*K(i,j) + ((bundle1.cnd/bundle1.delT) + (1/(bundle1.rf(1,j)*(bundle1.delX)^2)));
                 else
                     bundle1.rf(1,j) = bundle1.rf/theta(1,j);
            alpha_0(1,j) = bundle1.r0/bundle1.rf(1,j) ; 
alpha_N(1,j) = alpha_0(1,j)/(1 + bundle1.N*alpha_0(1,j));
e_nd(i,j) = alpha_N/(2*bundle1.rf(1,j)*(bundle1.delX)^2);
B_nd(i,j) = -2*e_nd(i,j);
...
% Begin time stepping
   ...
   % Step (i) of Reutskiy algorithm
       ...
       % Update the ionic current
            ...
            Dd = rightcreateReutskiy1vtheta(axon, bundle1, t,theta);
