...
injectnumber1 = 2; % axon number to be injected
...
axon = packagedCNNDec26vth(N, d, ell, L, delT, timesteps, endlength, temperature, Ranvier_array, r0, cm, gm, cnd, gnd, rf, InjectionDuration, injectnumber1, stimpos1, delaydelta, injectnumber2, stimpos2, delaydelta2, injectnumber3, stimpos3, delaydelta3, injectnumber4, stimpos4, Eonoff, Estrength, K3_1, th);
Timespent = toc
LTminus1 = timesteps;
...
for j = 1:endlength
    for plotnumber = 1:N
        subplot(N,1,plotnumber)
    if ismember(j,Ranvier_array)
       
plot([1:LTminus1].*delT, axon(plotnumber).V(j, 1:LTminus1))
axis([[1, LTminus1]*delT,-.1,.15]);
hold on;
grid on;
     end
