...
M = 5; % the grid size should be a `snug' fit to N.
N =3; % Number of axons in bundle; multiple of 3 for ease of entry of W matrix as a repmat of N=3 case; N < M
...
endlength = 300;%840; % total number of positions
...
ratio = .9;%0;%.5e1 % weakening the coupling to see the effect. %for very small ratios (1e-3) there is no interaction depolarization. for 1e-1 we begin to see interaction depolarization and for 1e0 there is full interaction depolarization
...
stimposarray = 20*ones(1,N);%[20,20,20,20,20.*ones(1,N-4)];
...
K3_1original = [(1+d11_1)^(p1), (1+d12_1)^(p1), (1+d13_1)^(p1); (1+d12_1)^(p1), (1+d22_1)^(p1), (1+d23_1)^(p1); (1+d13_1)^(p1), (1+d23_1)^(p1), (1+d33_1)^(p1)];
...
axon = packagedCNNDec24_2016v6(N, d, ell, L, delT, timesteps, endlength, temperature, Ranvier_array, r0, cm, gm, cnd, gnd, rf, InjectionDuration, injectnumberarraya, stimposarray, delaydeltarray, Eonoff, Estrength, K3_1, theta);
Timespent = toc
LTminus1 = timesteps;
% 
% % for count = 1:N
...
% SAVE OUTPUT %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
filename = strcat('numax', num2str(N), 'timedur', num2str(timesteps), 'axonlen', num2str(endlength), 'interres', num2str(r0), 'stimpos', num2str(stimpos1), 'date', date, 'injectnumberarray', num2str(injectnumberarraya(1,:)));

%save(filename, 'axon(1).V(:,:)', '-mat');
% 
% 
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % PLOT OUTPUT %
...
% Load your data (two time series)
% load('data.mat');
data = zeros(LTminus1,N);
data(:,1) = axon(1).V(20,1:LTminus1)';
data(:,2) = axon(2).V(20,1:LTminus1)';
data(:,3) = axon(3).V(20,1:LTminus1)';

% Choose a mother wavelet
wname = 'morl';

% Define the scales to use in the CWT
scales = 1:100;

% Perform the CWT on each time series
cwt1 = cwt(data(:,1), scales, wname);
cwt2 = cwt(data(:,2), scales, wname);
cwt3 = cwt(data(:,3), scales, wname);

cwt1 = reshape(cwt1,[],1);
cwt2 = reshape(cwt2,[],1);
cwt3 = reshape(cwt3,[],1);
% Calculate the wavelet cross-spectrum
[Wxy, F] = wcoherence(cwt1, cwt2);

% Calculate the wavelet power spectrum for each time series
Wx = abs(cwt1).^2;
Wy = abs(cwt2).^2;

% Calculate the wavelet coherence
Wcoh = abs(Wxy).^2 ./ (Wx' .* Wy');

% Plot the wavelet coherence
figure;
F = reshape(F, 1, []);
Wcoh = reshape(Wcoh,1,[]);
imagesc(F, scales, Wcoh);
set(gca, 'YDir', 'normal');
xlabel('Frequency (Hz)');
ylabel('Scale');
colorbar;
...
% Calculate the wavelet coherence
Wcoh2 = abs(Wyz).^2 ./ (Wy' .* Wz');

% Plot the wavelet coherence
figure(3);
F2 = reshape(F2, 1, []);
Wcoh2 = reshape(Wcoh2,1,[]);
imagesc(F2, scales, Wcoh2);
set(gca, 'YDir', 'normal');
xlabel('Frequency (Hz)');
ylabel('Scale');
colorbar;
