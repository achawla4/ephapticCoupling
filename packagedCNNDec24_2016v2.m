...
%%% Rev: Dec 27, 2015 %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function axon = packagedCNNDec24_2016v2(N, d, ell, L, delT, timesteps, endlength, temperature, Ranvier_array, r0, cm, gm, cnd, gnd, rf, InjectionDuration, injectnumber1, stimpos1, delaydelta, injectnumber2, stimpos2, delaydelta2, injectnumber3, stimpos3, delaydelta3, injectnumber4, stimpos4, Eonoff, Estrength, K)
...
% Begin time stepping
   ...
   % Step (i) of Reutskiy algorithm
      ...
      axon = ReutskiyStep2or7(leftmatrix, Dd, bundle1, axon, time,0);
       ...
       nodelookat = 20; % arbitrary node whose field will be plotted, initially
       numnodes = 1;
...
for i = 1:length(r)
    for j = 1:length(theta)
       for k= 1:length(phi)
        ...
        [Efieldx, Efieldy, Efieldz] = axonefield(numnodes, r(i), theta(j), phi(k), choice, 0, 0, mnow, nnow, hnow, pnow);
        [mycartcoordsx, mycartcoordsy, mycartcoordsz] = spher2cart(r(i), theta(j), phi(k)); % field positions in Cartesian coordinates
        coordstore{i,j,k,t} = [mycartcoordsx, mycartcoordsy, mycartcoordsz];
        fieldstore{i,j,k,t} = [Efieldx, Efieldy, Efieldz];
       end
    end
end
...
for i = 1:length(r)
    for j = 1:length(theta)
        for k = 1:length(phi)
        X(i,j,k,t) = 1e-2.*coordstore{i,j,k}(1);
        Y(i,j,k,t) = 1e-2.*coordstore{i,j,k}(2);
        Z(i,j,k,t) = 1e-2.*coordstore{i,j,k}(3);
        U(i,j,k,t) = (1/(1e6*(1/3e10))).*fieldstore{i,j,k,t}(1); % convert to SI volts per meter
        V(i,j,k,t) = (1/(1e6*(1/3e10))).*fieldstore{i,j,k,t}(2);
        W(i,j,k,t) = (1/(1e6*(1/3e10))).*fieldstore{i,j,k,t}(3);
        end
...
elseif i==2
    tf(L(1,t)>=1e6 & L(1,t) < 1e7) = true;
elseif i == 3
    tf(L(1,t)>=1e7 & L(1,t) < 5e7) = true;
else
     tf(L(1,t)>=5e7)=true;
end

     quiver3(X(tf,1:t),Y(tf,1:t),Z(tf,1:t),U(tf,1:t),V(tf,1:t),W(tf,1:t), 'color',cmap(i,:));
     drawnow
end
