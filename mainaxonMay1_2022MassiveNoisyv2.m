...
timesteps = 1500; % number of time steps. duration = timesteps*delT
endlength = 450;%840; % total number of positions
...
% For A only 
injectnumberarraya = [10,11,11,11,12,13,13,ones(1,N-15+8)];
...
ratio = .2;%0;%.5e1 % weakening the coupling to see the effect. %for very small ratios (1e-3) there is no interaction depolarization. for 1e-1 we begin to see interaction depolarization and for 1e0 there is full interaction depolarization
...
axon = packagedCNNDec24_2016v6v2(N, d, ell, L, delT, timesteps, endlength, temperature, Ranvier_array, r0, cm, gm, cnd, gnd, rf, InjectionDuration, injectnumberarraya, stimposarray, delaydeltarray, Eonoff, Estrength, myK3_1, theta);
Timespent = toc
LTminus1 = timesteps;
% 
% % for count = 1:N
