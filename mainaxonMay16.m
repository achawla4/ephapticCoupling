...
nodalspacing = 10;
Ranvier_array = [20:10:160,165:175,180:10:endlength-20];%positions of Ranvier nodes

ratio = 0.001;%0.5;%0;%.5e1 % weakening the coupling to see the effect. %for very small ratios (1e-3) there is no interaction depolarization. for 1e-1 we begin to see interaction depolarization and for 1e0 there is full interaction depolarization
...
axon = packagedCNNDec27(N, d, ell, L, delT, timesteps, endlength, temperature, Ranvier_array, r0, cm, gm, cnd, gnd, rf, InjectionDuration, injectnumber1, stimpos1, delaydelta, injectnumber2, stimpos2, delaydelta2, injectnumber3, stimpos3, delaydelta3, injectnumber4, stimpos4, Eonoff, Estrength);
Timespent = toc
LTminus1 = timesteps;
...
for i = 1:endlength
    ...
    if ismember(i,Ranvier_array)
       summedvoltage(i,1:LTminus1) = axon(1).V(i,1:LTminus1) + axon(2).V(i,1:LTminus1);
plot([1:LTminus1].*delT, summedvoltage(i, 1:LTminus1))
axis([[1, LTminus1]*delT,-.3,.5]);
hold on;
grid on;
    end
