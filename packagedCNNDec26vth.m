...
%%% Rev: Dec 27, 2015 %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function axon = packagedCNNDec26vth(N, d, ell, L, delT, timesteps, endlength, temperature, Ranvier_array, r0, cm, gm, cnd, gnd, rf, InjectionDuration, injectnumber1, stimpos1, delaydelta, injectnumber2, stimpos2, delaydelta2, injectnumber3, stimpos3, delaydelta3, injectnumber4, stimpos4, Eonoff, Estrength, K,th)
...
bundle1.r0 = r0*cos(th); %1e5; % interfiber medium resistance
...
bundle1.cm = cm/(cos(th))^2; %1.87e-11; % F/cm (both)
bundle1.gm = gm/(cos(th))^2; %5.6e-9; % per Ohm-cm (both)
bundle1.cnd = cnd/(cos(th))^2; %3.14e-9; % F/cm (from Waxman - Reutskiy has wrong dimensions)
...
% The above, so far, are the common parameters. 
 ...
 factor = 4/(cos(th))^2;%input('What value of injection intensity factor would you like to use? Suggest value is 1. ')
...
%      [axon(i).mcoeff(1:bundle1.endlength,1), axon(i).ncoeff(1:bundle1.endlength,1),  axon(i).hcoeff(1:bundle1.endlength,1),axon(i).pcoeff(1:bundle1.endlength,1)] = activationupdate(axon(i).m(1:bundle1.endlength,1), axon(i).n(1:bundle1.endlength,1), axon(i).h(1:bundle1.endlength,1), axon(i).p(1:bundle1.endlength,1), axon(i).Vcoeff(1:bundle1.endlength,1), Q, bundle1.delT);
    ...
    [axon(i).J_ion(1:bundle1.endlength,1), axon(i).J_Na(1:bundle1.endlength,1), axon(i).J_K(1:bundle1.endlength,1), axon(i).J_p(1:bundle1.endlength,1), axon(i).J_l(1:bundle1.endlength,1)] = ionicupdatevth(axon(i).m(1:bundle1.endlength,1),axon(i).n(1:bundle1.endlength,1),axon(i).h(1:bundle1.endlength,1),axon(i).p(1:bundle1.endlength,1),axon(i).Z_K(1:bundle1.endlength,1),axon(i).Z_Na(1:bundle1.endlength,1),V_l,axon(i).E(1:bundle1.endlength,1),bundle1.d, E_rest,th);
...
% Begin time stepping
   ...
   % Step (i) of Reutskiy algorithm
       ...
       % Update the ionic current
            ...
            [axon(j).J_ion(i,t), axon(j).J_Na(i,t), axon(j).J_K(i,t), axon(j).J_p(i,t), axon(j).J_l(i,t)]  = ionicupdatevth(axon(j).m(i,t),axon(j).n(i,t),axon(j).h(i,t),axon(j).p(i,t),axon(j).Z_K(i,t),axon(j).Z_Na(i,t),V_l,(axon(j).V(i,t)+E_rest),bundle1.d, E_rest,th);
      ...
      axon = ReutskiyStep2or7(leftmatrix, Dd, bundle1, axon, time,0);
       ...
       % Update the ionic-coeff current
            ...
            axnum = j;
            [axon(axnum).J_ioncoeff(i,time), axon(axnum).J_Nacoeff(i,time), axon(axnum).J_Kcoeff(i,time), axon(axnum).J_pcoeff(i,time), axon(axnum).J_lcoeff(i,time)] = ionicupdatevth(axon(axnum).mcoeff(i,time),axon(axnum).ncoeff(i,time),axon(axnum).hcoeff(i,time),axon(axnum).pcoeff(i,time),axon(axnum).Z_Kcoeff(i,time),axon(axnum).Z_Nacoeff(i,time),V_l,(axon(axnum).Ecoeff(i,time)),bundle1.d, E_rest,th);
