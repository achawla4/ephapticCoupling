...
% clf;
% clc;
function [v1newnew min1 max1 v2newnew min2 max2] = mainaxonerrorexp(x1,x2,ratio)
...
% h = 1e-21; % eventually time-varying
% gcostheta = (1-h)*cosd(oneightyminustheta);
...
axon = packagedCNNDec24_2016v6(N, d, ell, L, delT, timesteps, endlength, temperature, Ranvier_array, r0, cm, gm, cnd, gnd, rf, InjectionDuration, injectnumber1, stimpos1, delaydelta, injectnumber2, stimpos2, delaydelta2, injectnumber3, stimpos3, delaydelta3, injectnumber4, stimpos4, Eonoff, Estrength, K3_1, theta);
Timespent = toc
LTminus1 = timesteps;

v1 = axon(1).V(x1,:);

% find the min and max of v1
[min1 mindex1] = min(v1)


% if min1 is less than 0, shift v1 up rightward by min1
if min1<=0
    v1new = v1+abs(min1);
end
% if min1 is greater than 0, shift v1 down leftward by min1
if min1 > 0
    v1new = v1-abs(min1);
end

[max1 maxdex1] = max(v1new)

% if max1 is greater than 1, scale v1new down by max1
if max1 > 1
    v1newnew = v1new/abs(max1);
end
% if max1 is less than 1, scale v1new up by max1
if max1 <=1
    v1newnew = v1new*abs(max1);
end
...
% find the min and max of v2
[min2 mindex2] = min(v2)


% if min2 is less than 0, shift v2 up rightward by min2
if min2<=0
    v2new = v2+abs(min2);
end
% if min2 is greater than 0, shift v2 down leftward by min2
if min2 > 0
    v2new = v2-abs(min2);
end

[max2 maxdex2] = max(v2new)

% if max1 is greater than 1, scale v1new down by max1
if max2 > 1
    v2newnew = v2new/abs(max2);
end
% if max1 is less than 1, scale v1new up by max1
if max2 <=1
    v2newnew = v2new*abs(max2);
