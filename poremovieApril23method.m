...
function returncellarray = poremovieApril23method(start, width, trialnum,b, loops,fieldzero, conductivity)
...
for j = 1:loops
        x{1,j} = pxnum;
        y{1,j} = pynum;
        field{1,j} = randomtrapezoid(fieldzero,start, width,1,b,b*j/loops).*sqrt(vecxnum.^2 + vecynum.^2);
end
