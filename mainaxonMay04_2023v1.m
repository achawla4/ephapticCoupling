...
nodalspacing = 0;
Ranvier_array = [20]; % positions of Ranvier nodes
...
cm =  0.05*1.87e-11/cosd(theta); % F/cm (both)
gm = 0;%5.6e-9/cosd(theta); % per Ohm-cm (both)
...
% % 
% figure(1)
% for i = 1:10:endlength
%     for plotnumber = 1:N
%         subplot(N,1,plotnumber)
% %     if ismember(i,Ranvier_array)
%        
% plot3([1:LTminus1].*delT, axon(plotnumber).V(i, 1:LTminus1), [1:LTminus1].*delT)
...
y = axon(1).V(50, 1:LTminus1)*sin(theta(i));
z = axon(1).V(50, 1:LTminus1)*cos(theta(i));
plot3(x,y,z);
hold on;
end
...
for timeinst = 1:10:endlength
    x = 1:LTminus1;
theta = linspace(0,pi/2,20);
for i = 1:20

y = axon(1).V(timeinst, 1:LTminus1)*sin(theta(i));
z = axon(1).V(timeinst, 1:LTminus1)*cos(theta(i));
plot3(x,y,z);
% surf(x,y,z)
hold on;
end
...
%     currFrame = getframe;
%     size(currFrame)
%     frames(timeinst) = currFrame;
%     writeVideo(vidObj,currFrame);
end
% close(vidObj);
% Play the sequence of plots as a movie
% movie(frames);
...
% Plot surface and set viewing angle
% figure;
% plot3(x,y,z)
% surf(X,Y,Z);
% view(-37.5,30); % Rotate about z-axis
% hold on;
